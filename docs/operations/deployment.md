...
| Graph calls fail after redeployment | App Service recreated / managed identity replaced | Graph client credentials are unchanged — safe. If the App Service was recreated, re-run `deploy-infra` to refresh Key Vault secrets and App Service KV references. |
